...
static int is_zero_double(long x);
...
INTERFACE INLINE int is_integer(pointer p)
{
	if (!is_number(p))
		return 0;
	if (num_is_integer(p) || (long)ivalue(p) == rvalue(p))
		return 1;
	return 0;
}
...
INTERFACE long rvalue(pointer p)
{
	return (!num_is_integer(p) ? (p)->_object._number.value.rvalue :
				     (long)(p)->_object._number.value.ivalue);
}
...
#define num_rvalue(n) (!n.is_fixnum ? (n).value.rvalue : (long)(n).value.ivalue)
...
static num num_intand(num a, num b)
{
	num ret = { 0 };
...
static num num_intior(num a, num b)
{
	num ret = { 0 };
...
static num num_intxor(num a, num b)
{
	num ret = { 0 };
...
static num num_intnot(num a)
{
	num ret = { 0 };

	ret.is_fixnum = a.is_fixnum;
	if (ret.is_fixnum) {
...
INTERFACE pointer mk_real(scheme *sc, long n)
{
	pointer x = get_cell(sc, sc->NIL, sc->NIL);
...
static int file_interactive(scheme *sc)
{
	return sc->file_i == 0 && sc->inport->_object._port->kind & port_file;
}
		   ...
		   pointer code)
{
	...
	if (nframes >= sc->dump_size) {
		...
		sc->dump_base =
			realloc(sc->dump_base, sizeof(struct dump_stack_frame) *
						       sc->dump_size);
	}
...
static pointer opexe_2(scheme *sc, enum scheme_opcodes op)
{
	...
	case OP_LOGNOT:
		x = car(sc->args);
		s_return(sc, mk_number(sc, num_intnot(nvalue(x))));
		break;
...
static pointer opexe_3(scheme *sc, enum scheme_opcodes op)
{
	...
	case OP_GEQ: /* >= */
		...
		case OP_GEQ:
			...
			break;
		default:
			snprintf(sc->strbuff, STRBUFFSIZE,
...
static pointer opexe_4(scheme *sc, enum scheme_opcodes op)
{
	...
	case OP_EFUSE_READ: {
		...
		x = car(sc->args);
		addr = nvalue(x).value.ivalue;
		s_return(sc, mk_integer(sc, plat_efuse_read_safe(addr)));
	} break;
...
static pointer opexe_5(scheme *sc, enum scheme_opcodes op)
{
	...
	case OP_RDVEC:
		/* sc->code=cons(sc,mk_proc(sc,OP_VECTOR),sc->value); s_goto(sc,OP_EVAL); Cannot be quoted */
		/* x=cons(sc,mk_proc(sc,OP_VECTOR),sc->value); s_return(sc,x); Cannot be part of pairs */
		/* sc->code=mk_proc(sc,OP_VECTOR); sc->args=sc->value; s_goto(sc,OP_APPLY); */
		sc->args = sc->value;
		s_goto(sc, OP_VECTOR);
		break;
...
{
	int i, n = ARRAY_SIZE(dispatch_table);
	pointer x;
