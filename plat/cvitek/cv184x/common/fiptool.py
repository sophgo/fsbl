...
class FIP:
    ...
    param2 = OrderedDict(
        [
            ...
            Entry.make("MONITOR_RUNADDR", 8, int),
            # ATF-BL32
            Entry.make("BL32_CKSUM", 4, int),
            Entry.make("BL32_LOADADDR", 8, int),
            Entry.make("BL32_SIZE", 4, int),
            Entry.make("BL32_RUNADDR", 8, int),
            # ATF-BLMCU
            Entry.make("BLMCU_CKSUM", 4, int),
            Entry.make("BLMCU_LOADADDR", 8, int),
            Entry.make("BLMCU_SIZE", 4, int),
            Entry.make("BLMCU_RUNADDR", 8, int),
            # u-boot
    ...
    def read_fip2(self, fip_bin):
        ...
        for e in self.param2.values():
            ...
            if hasattr(e, 'name'):
                if e.name.endswith('_SIZE'):
                    ...
                    elif comp_type == LOADER_2ND_MAGIC_LZMA:
                        logging.debug("BLCP_2ND compression: lzma")
        ...
        if self.param2["DDR_PARAM_SIZE"].toint() > 0:
            loadaddr = self.param2["DDR_PARAM_LOADADDR"].toint()
            size = self.param2["DDR_PARAM_SIZE"].toint()
            logging.debug(f"Reading DDR_PARAM: loadaddr=0x{loadaddr:x}, size=0x{size:x}")
            self.body2["DDR_PARAM"].content = fip_bin[loadaddr : loadaddr + size]
            self.read_end = max(self.read_end, loadaddr + size)

        if self.param2["BLCP_2ND_COMP_SIZE"].toint() > 0:
            ...
            self.blcp_2nd_info = {
                ...
                'comp_addr': self.param2["BLCP_2ND_COMP_ADDR"].toint()
            }
        elif self.param2["BLCP_2ND_SIZE"].toint() > 0:
            loadaddr = self.param2["BLCP_2ND_LOADADDR"].toint()
            ...
            logging.debug("No BLCP_2ND found")
            self.blcp_2nd_info = None
           
        for name in [ "MONITOR", "BL32", "BLMCU"]:
            size = self.param2[name + "_SIZE"].toint()
            loadaddr = self.param2[name + "_LOADADDR"].toint()
            if size > 0:
                logging.debug(f"Reading {name}: loadaddr=0x{loadaddr:x}, size=0x{size:x}")
                try:
                    self.body2[name].content = fip_bin[loadaddr : loadaddr + size]
                    logging.debug(f"Successfully read {name}, content size=0x{len(self.body2[name].content):x}")
                    self.read_end = max(self.read_end, loadaddr + size)
                except Exception as e:
                    logging.error(f"Failed to read {name}: {str(e)}")

        loader_2nd_loadaddr = self.param2["LOADER_2ND_LOADADDR"].toint()
        if loader_2nd_loadaddr:
            logging.debug(f"Reading LOADER_2ND at 0x{loader_2nd_loadaddr:x}")
            try:
                self._parse_ldr_2nd_hdr(fip_bin[loader_2nd_loadaddr:])
                ldr_2nd_size = self.ldr_2nd_hdr["SIZE"].toint()
                logging.debug(f"LOADER_2ND size=0x{ldr_2nd_size:x}")
    ...
    def add_blmcu(self, args):
        with open(args.BLMCU, "rb") as fp:
            blmcu = fp.read()

        logging.debug("blmcu=%#x bytes", len(blmcu))
        self.body2["BLMCU"].content = blmcu
        ...
        try:
            if hasattr(self, 'blcp_2nd_info') and self.blcp_2nd_info:
                ...
                if args:
                    ...
                    buildin = args.BLCP_2ND_BUILDIN if hasattr(args, 'BLCP_2ND_BUILDIN') else "n"
                    decompaddr = self.param2["BLCP_2ND_COMP_ADDR"].toint()
                else:
                    logging.error("No BLCP_2ND info available")
                    return fip_bin

            if comp_type == "orig":
                self.param2["BLCP_2ND_COMP_TYPE"].content = LOADER_2ND_MAGIC_ORIG
            elif comp_type == "lzma":
            ...
            if buildin == "y":
                ...
                if self.param2["BLCP_2ND_COMP_TYPE"].content == LOADER_2ND_MAGIC_ORIG:
                    ...
                    self.param2["BLCP_2ND_COMP_SIZE"].content = 0
                else:
                    self.param2["BLCP_2ND_SIZE"].content = 0
                    self.param2["BLCP_2ND_COMP_SIZE"].content = len(body)
                return fip_bin + body
            else:
                self.param2["BLCP_2ND_LOADADDR"].content = 0
                if self.param2["BLCP_2ND_COMP_TYPE"].content == LOADER_2ND_MAGIC_ORIG:
                    self.param2["BLCP_2ND_SIZE"].content = len(body)
                    self.param2["BLCP_2ND_COMP_SIZE"].content = 0
                else:
            ...
            logging.error(f"Error in pack_blcp_2nd: {str(e)}")
    ...
    def pack_monitor(self, fip_bin, monitor_runaddr):
        ...
        logging.debug(f"Input fip_bin type: {type(fip_bin)}")
        
        if not isinstance(fip_bin, (bytes, bytearray)):
            logging.error(f"Invalid fip_bin type: {type(fip_bin)}")
            return bytearray()
        
        if not isinstance(fip_bin, bytearray):
            fip_bin = bytearray(fip_bin)
        
        if not self.body2["MONITOR"].content:
            logging.debug("No MONITOR content")
            return fip_bin
            
        try:
            monitor_content = self.body2["MONITOR"].content
            logging.debug(f"MONITOR content length: {len(monitor_content)}")
            
            if not isinstance(monitor_content, bytearray):
                monitor_content = bytearray(monitor_content)
                
            fip_bin = self.pad(fip_bin, IMAGE_ALIGN)
            monitor = self.pad(monitor_content, IMAGE_ALIGN)
            ...
            self.param2["MONITOR_RUNADDR"].content = int(monitor_runaddr)
            
            result = fip_bin + monitor
            logging.debug(f"Final result type: {type(result)}, length: {len(result)}")
            return result
            
        except Exception as e:
            logging.error(f"Error in pack_monitor: {str(e)}")
            logging.error(f"Error type: {type(e)}")
            import traceback
            logging.error(f"Traceback: {traceback.format_exc()}")
            return fip_bin
    ...
    def pack_blmcu(self, fip_bin, blmcu_runaddr):
        logging.debug("pack_blmcu:")
        if not len(self.body2["BLMCU"].content):
            return

        blmcu_runaddr = int(blmcu_runaddr)

        fip_bin = self.pad(fip_bin, IMAGE_ALIGN)

        # Pack BLMCU to body2
        blmcu = self.pad(self.body2["BLMCU"].content, IMAGE_ALIGN)

        self.param2["BLMCU_CKSUM"].content = self.image_crc(blmcu)
        self.param2["BLMCU_SIZE"].content = len(blmcu)
        self.param2["BLMCU_LOADADDR"].content = len(fip_bin)
        self.param2["BLMCU_RUNADDR"].content = blmcu_runaddr

        return fip_bin + blmcu
    ...
    def append_fip2(self, fip1_bin, args):
        ...
        logging.debug(f"param2_loadaddr: {param2_loadaddr}, type: {type(param2_loadaddr)}")
        ...
        if len(self.body2["BLCP_2ND"].content):
            ...
            if tmp is not None: 
                ...
                logging.warning("pack_blcp_2nd returned None, using original fip_bin")
        ...
        if len(self.body2["BLMCU"].content):
            runaddr = self.param2["BLMCU_RUNADDR"].toint()
            if not runaddr:
                runaddr = int(args.BLMCU_RUNADDR)
            fip_bin = self.pack_blmcu(fip_bin, runaddr)
...
def parse_args():
    ...
    pr_gen.add_argument("--BLCP_2ND_BUILDIN", choices=["y", "n", ""])
    ...
    pr_gen.add_argument("--BLMCU_RUNADDR", type=auto_int)
