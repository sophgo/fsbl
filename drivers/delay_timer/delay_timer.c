...
void udelay_skippable_in_sim(uint32_t usec)
{
	assert(ops != NULL &&
		(ops->clk_mult != 0) &&
		(ops->clk_div != 0) &&
...
void mdelay_skippable_in_sim(uint32_t msec)
{
	udelay_skippable_in_sim(msec * 1000);
}
...
uint32_t get_timer(uint32_t base)
{
	...
	if (base == 0)
		...
		return (base - ops->get_timer_value()) * ops->clk_mult / ops->clk_div / 1000; // ms
}
...
uint32_t get_random_from_timer(uint32_t base)
{
	...
	if (base == 0)
		...
		return (base - ops->get_timer_value()) * ops->clk_mult / ops->clk_div % 100000;
}
