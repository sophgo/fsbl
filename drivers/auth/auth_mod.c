		      ...
		      void *img, unsigned int img_len)
{
	...
	unsigned int cert_nv_ctr, plat_nv_ctr, cv_cert_nv_ctr;
	unsigned int plat_market_segment, cv_market_segment;
	int rc = 0;
...
#if 0 // CV uses high 16-bits for market segment
	...
	INFO("img_id %d, cert_nv_ctr %u\n", img_desc->img_id, cert_nv_ctr);

	/* Get the counter from the platform */
	rc = plat_get_nv_ctr(param->plat_nv_ctr->cookie, &plat_nv_ctr);
	return_if_error(rc);
	if (img_desc->img_id == BL33_KEY_CERT_ID || img_desc->img_id == BL33_CONTENT_CERT_ID)
		plat_nv_ctr = (plat_nv_ctr >> 16) & 0xFFFF;
	else
		plat_nv_ctr = plat_nv_ctr & 0xFFFF;
	INFO("plat_nv_ctr %x\n", plat_nv_ctr);

	cv_cert_nv_ctr = cert_nv_ctr & 0xFFFF;
	INFO("cv_cert_nv_ctr %x\n", cv_cert_nv_ctr);
	if (cv_cert_nv_ctr < plat_nv_ctr) {
		/* Invalid NV-counter */
		ERROR("Invalid NV-counter 0x%x\n", cv_cert_nv_ctr);
		return 1;
	}
	INFO("NV-counter is valid\n");

	rc = plat_get_market_segment(&plat_market_segment);
	return_if_error(rc);
	if (img_desc->img_id == BL33_KEY_CERT_ID || img_desc->img_id == BL33_CONTENT_CERT_ID)
		plat_market_segment = (plat_market_segment >> 16) & 0xFFFF;
	else
		plat_market_segment = plat_market_segment & 0xFFFF;
	INFO("plat_market_segment %x\n", plat_market_segment);

	cv_market_segment = (cert_nv_ctr >> 16) & 0xFFFF;
	INFO("cv_market_segment %x\n", cv_market_segment);
	if (cv_market_segment != plat_market_segment) {
		/* Invalid market segment */
		ERROR("Invalid market segment 0x%x\n", cv_market_segment);
		return 1;
	}
	INFO("Market segment is valid\n");
			...
			unsigned int img_len)
{
	...
	INFO("auth_mod_verify_img img_id %u\n", img_id);
