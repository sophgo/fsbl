...
void ddr_sys_bring_up(void)
{
	...
	// // meas_en[0] = 0
	// rddata = mmio_rd32(0x03000140);

	// rddata = modified_bits_by_value(rddata, 7, 7, 2);//meas_sel=mpll
	// rddata = modified_bits_by_value(rddata, 0, 0, 0);//mon_en disable
	// mmio_wr32(0x03000140, rddata);
	// rddata = modified_bits_by_value(rddata, 1, 0, 0);//mon_en enable
	// mmio_wr32(0x03000140, rddata);
	...
	// meas_count = (rddata >> 8) & 0x3FFFF;
	// clk_in_mhz = meas_count / 10;

	// NOTICE("0x03000140=0x%x, clk_in_mhs=%d\n", rddata, clk_in_mhz);

	// // meas_en[0] = 0
	// rddata = mmio_rd32(0x03000140);

	// rddata = modified_bits_by_value(rddata, 8, 7, 2);//meas_sel=mpll
	// rddata = modified_bits_by_value(rddata, 0, 0, 0);//mon_en disable
	// mmio_wr32(0x03000140, rddata);
	// rddata = modified_bits_by_value(rddata, 1, 0, 0);//mon_en enable
	// mmio_wr32(0x03000140, rddata);
	...
	KC_MSG(", bist_result = %x, err_data_odd = %lx, err_data_even = %lx\n", bist_result, err_data_odd,
	       err_data_even);
	if (bist_result == 0) {
		ERROR("ERROR bist_fail before WR\n");
	...
	KC_MSG(", bist_result = %x, err_data_odd = %lx, err_data_even = %lx\n", bist_result, err_data_odd,
	       err_data_even);
	if (bist_result == 0) {
		ERROR("ERROR bist_fail before rdg\n");
		ERROR("bist_result = %x, err_data_odd = %lx, err_data_even = %lx\n", bist_result, err_data_odd,
			  err_data_even);
	}
#endif
	...
	KC_MSG(", bist_result = %x, err_data_odd = %lx, err_data_even = %lx\n", bist_result, err_data_odd,
	       err_data_even);
	if (bist_result == 0) {
		ERROR("ERROR bist_fail after rdg\n");
		ERROR("bist_result = %x, err_data_odd = %lx, err_data_even = %lx\n", bist_result, err_data_odd,
			  err_data_even);
	}
#endif
