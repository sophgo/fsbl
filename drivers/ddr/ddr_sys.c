...
void shmoo_print(uint32_t var, uint8_t len)
{
	for (int i = len; i > 0; i--) {
		SHMOO_MSG("%x", get_bits_from_value(var, 4 * i - 1, 4 * (i - 1)));
	}
}
...
void cvx16_synp_mrw(uint32_t addr, uint32_t data)
{
	...
	mmio_wr32(cfg_base + 0x18, 0x80000000);
		...
		while (1) {
			...
			if (((byte0_pirdlvl_sw_upd_ack == 0x1) || (byte0_all_le_found & byte0_all_te_found)) &&
				...
				if (((byte0_pirdlvl_sw_upd_ack == 0x1) || (byte1_pirdlvl_sw_upd_ack == 0x1)) &&
					...
					SHMOO_MSG("vref = ");
					shmoo_print(rx_vref_sel, 2);
					SHMOO_MSG(", sw_rdq_training_start = ");
					shmoo_print(dlie_code, 8);
					SHMOO_MSG(" , ");
					...
					SHMOO_MSG("err_data_rise/err_data_fall = ");
					shmoo_print(((byte0_data_rise_fail & 0x000000FF) |
							((byte1_data_rise_fail & 0x000000FF) << 8)), 8);
					SHMOO_MSG(", ");
					shmoo_print(((byte0_data_fall_fail & 0x000000FF) |
							((byte1_data_fall_fail & 0x000000FF) << 8)), 8);
					SHMOO_MSG("\n");
				}
				//if (((byte0_pirdlvl_sw_upd_ack == 0x1) || (byte1_pirdlvl_sw_upd_ack == 0x1))
				//    || ((byte0_cur_pirdlvl_st != 0x0) && (byte1_cur_pirdlvl_st != 0x0))) {
...
// GPO setting
	...
	// SSC_EN =0
	//==============================================================
	rddata = mmio_rd32(0x40 + 0x03002900); // TOP_REG_SSC_SET
	rddata = modified_bits_by_value(rddata, 1, 0, 0);
	mmio_wr32(0x40 + 0x03002900, rddata);
	//NOTICE("0x%x = 0x%x\n", 0x40 + 0x03002900, mmio_rd32(0x40 + 0x03002900));

	uartlog("SSC_EN =0\n");
	rddata = reg_set; // TOP_REG_SSC_SET
	mmio_wr32(0x54 + 0x03002900, rddata);
	//NOTICE("0x%x = 0x%x\n", 0x54 + 0x03002900, mmio_rd32(0x54 + 0x03002900));
	rddata = get_bits_from_value(reg_span, 15, 0); // TOP_REG_SSC_SPAN
	mmio_wr32(0x58 + 0x03002900, rddata);
	//NOTICE("0x%x = 0x%x\n", 0x58 + 0x03002900, mmio_rd32(0x58 + 0x03002900));
	rddata = get_bits_from_value(reg_step, 23, 0); // TOP_REG_SSC_STEP
	mmio_wr32(0x5C + 0x03002900, rddata);
	//NOTICE("0x%x = 0x%x\n", 0x5c + 0x03002900, mmio_rd32(0x5c + 0x03002900));

	//NOTICE("0x%x = 0x%x\n", 0x50 + 0x03002900, mmio_rd32(0x50 + 0x03002900));
	rddata = mmio_rd32(0x50 + 0x03002900);
	rddata = modified_bits_by_value(rddata, ~get_bits_from_value(rddata, 0, 0), 0, 0); // TOP_REG_SSC_SW_UP
	rddata = modified_bits_by_value(rddata, 0, 1, 1); // TOP_REG_SSC_EN_SSC
	rddata = modified_bits_by_value(rddata, 0, 3, 2); // TOP_REG_SSC_SSC_MODE
	...
	mmio_wr32(0x0C + CV_DDR_PHYD_APB, rddata);
	//NOTICE("0x%x = 0x%x\n", 0x0C + CV_DDR_PHYD_APB, mmio_rd32(0x0C + CV_DDR_PHYD_APB));
	rddata = mmio_rd32(0x10 + CV_DDR_PHYD_APB);
	...
	//NOTICE("0x%x = 0x%x\n", 0x10 + CV_DDR_PHYD_APB, mmio_rd32(0x10 + CV_DDR_PHYD_APB));
	//[0]   = 1;    //TOP_REG_RESETZ_DIV
	rddata = 0x1;
	mmio_wr32(0x04 + CV_DDR_PHYD_APB, rddata);
	//NOTICE("0x%x = 0x%x\n", 0x04 + CV_DDR_PHYD_APB, mmio_rd32(0x04 + CV_DDR_PHYD_APB));
	uartlog("RSTZ_DIV=1\n");
	rddata = mmio_rd32(0x0C + CV_DDR_PHYD_APB);
...
static uint32_t bist_all_dram_calvl(uint32_t mode, uint32_t capacity, uint32_t shift_delay, uint32_t vrefca)
{
	...
	SHMOO_MSG_CA("vref = ");
	shmoo_print(vrefca, 2);
	SHMOO_MSG_CA(", sw_ca__training_start = ");
	shmoo_print(shift_delay, 8);
	SHMOO_MSG_CA(" , err_data_rise/err_data_fall = ");
	shmoo_print(bist_err, 8);
	SHMOO_MSG_CA(", ");
	shmoo_print(bist_err, 8);
	SHMOO_MSG_CA("\n");
...
static uint32_t bist_all_dram_cslvl(uint32_t mode, uint32_t capacity, uint32_t shift_delay, uint32_t vrefca)
{
	...
	SHMOO_MSG_CS("vref = ");
	shmoo_print(vrefca, 2);
	SHMOO_MSG_CS(", sw_cs__training_start = ");
	shmoo_print(shift_delay, 8);
	SHMOO_MSG_CS(" , err_data_rise/err_data_fall = ");
	shmoo_print(bist_err, 8);
	SHMOO_MSG_CS(", ");
	shmoo_print(bist_err, 8);
	SHMOO_MSG_CS("\n");
