...
#define FLAGS_FEATURE_ECC_CONFIG	(BIT(4)) // overwrite setting from sysvec
...
uint32_t bm_spi_nand_read_page_by_row_addr(uint32_t row_addr, void *buf, uint32_t len);
#ifdef NANDBOOT_V2
uint32_t bm_spi_nand_read_oob(uint32_t row_addr, void *buf);
#endif
void bm_spi_nand_set_device(spi_nand_info_t *info);
spi_nand_info_t *bm_spi_nand_get_info(void);
void bm_spi_nand_dump_dma_reg(void);
