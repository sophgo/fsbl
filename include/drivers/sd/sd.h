...
#define SD_FLAG_CMD23			(1 << 0)
#define SD_FLAG_HOST_RESET		(1 << 1)
#define SD_FLAG_HAVE_PHY		(1 << 2)
#define SD_FLAG_INIT_PHY		(1 << 3)

#define SD_INIT_DELAY_TIME_START (10)		// ms
#define SD_INIT_DELAY_TIME_INC (40)		// ms

#define SD_INIT_DELAY_TIME_MAX (250)
...
size_t sd_read_blocks(int lba, uintptr_t buf, size_t size);
size_t sd_write_blocks(int lba, const uintptr_t buf, size_t size);
size_t sd_erase_blocks(int lba, size_t size);
size_t sd_partition_read_blocks(sd_partition_type type,int lba, uintptr_t buf, size_t size);
size_t sd_partition_write_blocks(sd_partition_type type,int lba, const uintptr_t buf, size_t size);
size_t sd_partition_erase_blocks(sd_partition_type type,int lba, size_t size);
void sd_init(const sd_ops_t *ops, int clk, int bus_width,unsigned int flags);
size_t sd_read_block_cmd8(int lba, uintptr_t buf, size_t size);
size_t sd_boot1_read_blocks(int lba, uintptr_t buf, size_t size);
size_t sd_boot1_write_blocks(int lba, uintptr_t buf, size_t size);
void sd_ut();

#if defined(__CVITEK__) && !defined(__ASSEMBLY__)
#include <cv_sd.h>
#include <cv_private.h>
#elif defined(__BITMAIN__) && !defined(__ASSEMBLY__)
#include <bm_sd.h>
