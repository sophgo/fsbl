...
int plat_get_prg_image_source(unsigned int image_id,
			uintptr_t *dev_handle,
			uintptr_t *image_spec);
#endif
...
struct meminfo *bld_plat_sec_mem_layout(void);
...
image_desc_t *bl1_plat_get_bld_image_desc(unsigned int image_id);

#ifdef LOAD_PRG_BIN_FROM_SDCARD
struct image_desc *bl1_plat_get_prg_bin_image_desc(unsigned int image_id);
#endif
...
struct meminfo *bl2_plat_ns_dram_layout(void);
