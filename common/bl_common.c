...
#if defined(__CVITEK__)
#include <cv_private.h>
#elif defined(__BITMAIN__)
	       ...
	       entry_point_info_t *entry_point_info)
{
...
#if (defined(IMAGE_BL1) && defined(BL1_INIT_SDFAT) && defined(BL1_INIT_NFC))
	if (image_id == PRG_BIN_IMAGE_ID)
	{
		NOTICE("load_image : load prg.bin and get sd dev_handle and image_spec\n");
		io_result = plat_get_prg_image_source(image_id, &dev_handle, &image_spec);
	}
	else
#endif
	{
	...
	if (plat_is_trusted_boot() == 0) {
		...
		i = crc16_ccitt(0, (unsigned char *)image_base, image_size);
		if (i != (signature&0xFFFF)) {
			io_result = -EBADF;
			ERROR("Image id_%u: data corruption (crc=0x%x, wrong-crc=0x%x)\n",
				image_id, (signature&0xFFFF), i);
			goto exit;
		}
		INFO("Image id_%u: CRC check pass! (0x%x @0x%lx)\n",
			image_id, signature, image_base+image_size);
	}
#endif
...
#if (defined(IMAGE_BL1) && defined(BL1_INIT_SDFAT) && defined(BL1_INIT_NFC))
int load_prg_image(meminfo_t *mem_layout,
					unsigned int image_id,
					uintptr_t image_base,
					image_info_t *image_data,
					entry_point_info_t *entry_point_info)
{
	...
	/* Obtain a reference to the image by querying the platform layer */
	io_result = plat_get_prg_image_source(image_id, &backend_dev_handle,
				       &backend_image_spec);
	...
	/* Attempt to access the image */
	io_result = io_open(backend_dev_handle, backend_image_spec, &image_handle);
	if (io_result != 0) {
		WARN("Failed to access image id=%u (%i)\n",
			image_id, io_result);
		return io_result;
				    ...
				    int is_parent_image)
{
	int rc;
#if TRUSTED_BOARD_BOOT
	int is_trusted_boot = plat_is_trusted_boot();
#if defined(IMAGE_BL2)
	if (image_id == BL33_IMAGE_ID)
		is_trusted_boot = plat_ree_is_trusted_boot();
#endif /* defined(IMAGE_BL2) */
	NOTICE("C%d:%d.", image_id, is_trusted_boot);
#endif /* TRUSTED_BOARD_BOOT */
	...
	if (is_trusted_boot) {
		...
		if (rc != 0) {
			/*
			 * Authentication error, zero memory
			...
			flush_dcache_range(image_data->image_base,
				   image_data->image_size);
			ERROR("Authenticate image_id #%d failed\n", image_id);
			/* TBBR (R0110) ask to launch FW update routine when TOC is not valid
			 * here we trigger WDT and then reset for user to plugin USB cable
			 * or SD card to enter FW update mode
			 */
			bm_plat_reboot();
			return -EAUTH;
		}
		    ...
		    entry_point_info_t *entry_point_info)
{
	...
	rc = load_auth_image_internal(mem_layout, image_id, image_base,
						image_data, entry_point_info, 0);
	if (!rc) {
#if defined(IMG_ENC)
		if (plat_blx_is_encrypted(image_id)) {
			if ((image_id == BL2_IMAGE_ID) || (image_id == BL31_IMAGE_ID) ||
				(image_id == BL32_IMAGE_ID) || (image_id == BL33_IMAGE_ID) ||
				(image_id == BLD_IMAGE_ID)) {

				size_t image_size = image_data->image_size;
				uintptr_t image_base_2 = image_data->image_base;
				mbedtls_aes_context aes;
				uint32_t dis_hw_dec_img = plat_bm_gpio_read(BIT_MASK_GPIO_HW_DEC);
				int cbc_ret;

				unsigned char key[16] __aligned(16) = {0x00};
				unsigned char iv[16] __aligned(16) = {0x00};

				NOTICE("E%u.", image_id);
				get_blx_dec_key(image_id, key);

				if (dis_hw_dec_img == 1) {
					mbedtls_aes_init(&aes);
					mbedtls_aes_setkey_dec(&aes, key, 128);

					cbc_ret = mbedtls_aes_crypt_cbc(&aes, MBEDTLS_AES_DECRYPT, image_size, iv,
							(unsigned char *) image_base_2, (unsigned char *) image_base_2);
					INFO("decrypting... imageID:%d, imageBase:%p, imageSize:%d, cbc_ret = 0x%s%x\n",
						image_id, (void *)image_base_2, (unsigned int)image_size,
						cbc_ret < 0 ? "-" : "", cbc_ret < 0 ? -(unsigned int)cbc_ret:cbc_ret);
				} else {
					cbc_ret = plat_cryptodma_des(image_base_2, image_base_2, image_size, key, iv, DES_USE_AES);
					INFO("decrypting (dma)... imageID:%d, imageBase:%p, imageSize:%d\n",
					     image_id, (void *)image_base_2,
					     (unsigned int)image_size);
					INFO("cbc_ret = 0x%s%x\n",
					     cbc_ret < 0 ? "-" : "",
					     cbc_ret < 0 ?
						     -(unsigned int)cbc_ret :
						     cbc_ret);
				}
				...
				if (!cbc_ret) {
					/* Flush the image to main memory so that it can be executed later by */
					/* any CPU */
					INFO("flush decrypted image, image_id=%d, image_base=%p, image_size=0x%x\n",
						(int)image_id, (void *)image_base, (int)image_size);
					flush_dcache_range(image_base, image_size);
				}
			}
		}
...
#if defined(IMAGE_BL2)
	if (!rc && image_id == BL33_IMAGE_ID) {
		size_t uncomp_image_size = image_data->image_size;

		NOTICE("F%u.", image_id);
		rc = lzma_decompress(image_data->image_base, image_data->image_size, &uncomp_image_size);
		image_data->image_size = uncomp_image_size;
