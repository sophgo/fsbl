...
fip%: export BLCP_2ND_BUILDIN ?= y
fip%: export BLCP_2ND_COMP_TYPE ?= orig
${BUILD_PLAT}:
	@mkdir -p '${BUILD_PLAT}'
...
blmacros-env: blmacros
	...
	${Q}$(call macro_to_env,BLCP_2ND_COMP_ADDR)
...
fip-all: fip-dep
	...
	${FIPTOOL} -v genfip \
		...
		--BLCP_2ND_BUILDIN="$${BLCP_2ND_BUILDIN}" \
		--BLCP_2ND_COMP_TYPE="$${BLCP_2ND_COMP_TYPE}" \
		--BLCP_2ND_COMP_ADDR="$${BLCP_2ND_COMP_ADDR}" \
		--CHIP_CONF='${CHIP_CONF_PATH}' \
		--NOR_INFO='${NOR_INFO}' \
		--NAND_INFO='${NAND_INFO}'\
		--BL2='${BUILD_PLAT}/bl2.bin' \
