...
$(OBJ): $(2) | bl$(3)_dirs
	@echo "  AS      $$<"
	$$(Q)$$(AS) $$(BL$(3)_ASFLAGS) -D$(IMAGE) $(MAKE_DEP) -c $$< -o $$@
...
$(1): $(2) | bl$(3)_dirs
	@echo "  PP      $$<"
	$$(Q)$$(CPP) $$(BL$(3)_CPPFLAGS) -P -D__ASSEMBLY__ -D__LINKER__ $(MAKE_DEP) -o $$@ $$<
...
$(eval EL_CFLAGS := $(BL31_CFLAGS))

$(OBJ): ${EXTRA_DEPS}
$(OBJ): $(2) | bl$(3)_dirs
	@echo "  CC      $$<"
	$$(Q)$$(CC) $$(EL_CFLAGS) $$(CFLAGS) -D$(IMAGE) $(MAKE_DEP) -c $$< -o $$@
...
$(OBJ): $(2) | bl$(3)_dirs
	@echo "  AS      $$<"
	$$(Q)$$(AS) $$(BL31_ASFLAGS) -D$(IMAGE) $(MAKE_DEP) -c $$< -o $$@
...
$(1): $(2) | bl$(3)_dirs
	@echo "  PP      $$<"
	$$(Q)$$(CPP) $$(BL31_CPPFLAGS) -P -D__ASSEMBLY__ -D__LINKER__ $(MAKE_DEP) -o $$@ $$<
...
#   $(2) = list of source files (both C and assembly)
#   $(3) = BL stage (2, 30, 31, 32, 33)
define BL31_MAKE_OBJS
        $(eval C_OBJS := $(filter %.c,$(2)))
        $(eval REMAIN := $(filter-out %.c,$(2)))
        $(eval $(foreach obj,$(C_OBJS),$(call BL31_MAKE_C,$(1),$(obj),$(3))))

        $(eval S_OBJS := $(filter %.S,$(REMAIN)))
        $(eval REMAIN := $(filter-out %.S,$(REMAIN)))
        $(eval $(foreach obj,$(S_OBJS),$(call BL31_MAKE_S,$(1),$(obj),$(3))))
...
$(ELF): $(OBJS) $(LINKERFILE) | bl$(1)_dirs
	...
	@echo 'const char build_message[] = $(BUILD_MESSAGE_TIMESTAMP); \
		...
		$$(CC) $$(BL$(1)_CFLAGS) $$(CFLAGS) -xc -c - -o $(BUILD_DIR)/build_message.o
